...
export default function DashboardAnalytics({ results, customHistory }) {
    ...
    Object.entries(results).forEach(([category, apis]) => {
      apis.forEach((api) => {
        list.push({
          ...
          success: api.success,
          error: api.error || '',
          dataSnippet: api.success ? JSON.stringify(api.data) : '',
  ...
  // Calculate total transactions and success ratio
  const totalTransactions = totalCounts.reduce((acc, cur) => acc + cur.pass + cur.fail, 0);
  const totalPass = totalCounts.reduce((acc, cur) => acc + cur.pass, 0);
  const successRatio = totalTransactions === 0 ? 0 : (totalPass / totalTransactions) * 100;
  ...
  const downloadPDF = async () => {
    try {
      ...
      // Chart image at top
      pdf.addImage(chartImg, 'PNG', 10, 20, 190, 80);
  ...
  const exportToExcel = () => {
    ...
    // Add customHistory rows for Excel too
    customHistory.forEach(api => {
      rows.push({
        Category: 'Custom',
        'API Name': `${api.method} ${api.name}`,
        'Status Code': api.status,
        Result: api.success ? 'PASS' : 'FAIL',
        'Response / Error': api.success ? JSON.stringify(api.data) : api.error,
        'Correlation ID': api.correlationId || '',
    ...
    <div className="kpi-container">
      <div className="kpi-card">
        Total Transactions<br />
          <div className="kpi-value">{allApis.length} 
          </div>
      </div>

          <div className="kpi-card success-ratio">
      ...
      <div className="analytics-table-container">
        <table className="analytics-table">
          ...
          <tbody>
            {allApis.length === 0 ? (
              <tr>
                <td colSpan={6} className="center" style={{ padding: '12px' }}>
                  No API results yet
                </td>
              </tr>
            ) : (
